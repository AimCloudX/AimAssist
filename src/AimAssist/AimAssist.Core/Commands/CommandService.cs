...
namespace AimAssist.Core.Commands
{
    ...
    {
        private Dictionary<string, KeySequence?> keymap = new();
        private List<RelayCommand> dic = [];
        ...
        public void SetKeymap(Dictionary<string, KeySequence?> maps)
        {
            ...
            {
                if (keymap.TryGetValue(map.Key, out _))
                {
        ...
        public bool TryGetFirstOnlyKeyCommand(KeyGesture keyGesture, out RelayCommand? command)
        {
            foreach (var keyValuePair in keymap)
            {
                ...
                if (keySequence != null && keySequence.FirstKey == keyGesture.Key && keySequence.FirstModifiers == keyGesture.Modifiers && keySequence.SecondKey == null && keySequence.SecondModifiers == null)
                {
        ...
        public bool TryGetFirstSecontKeyCommand(KeySequence input, out RelayCommand? command)
        {
            ...
            {
                var keySequence = keyValuePair.Value;
                if (keySequence != null && keySequence.FirstKey == input.FirstKey && keySequence.FirstModifiers == input.FirstModifiers && keySequence.SecondKey == input.SecondKey && keySequence.SecondModifiers == input.SecondModifiers)
                {
                    if (TryGetCommand(keyValuePair.Key, out command))
                    {
                        return true;
        ...
        public void UpdateKeyGesture(string commandName, KeySequence? key)
        {
            ...
            if (!keymap.TryGetValue(commandName, out var before)) return;
            keymap[commandName] = key;
            if (command is not HotkeyCommand hotkeyCommand) return;
            if (before != null) EventPublisher.KeyUpdateEventPublisher.RaiseEvent(hotkeyCommand, before, key);
        }
