...
namespace AimPicker.Service.HotKeys;
...
public class HotKeyController : IDisposable
{
    ...
    public HotKeyController(Window window)
    {
        ...
        ComponentDispatcher.ThreadPreprocessMessage += this.ComponentDispatcher_ThreadPreprocessMessage;
    }
    ...
    public bool Unregister(ModifierKeys modKey, Key key)
    {
        ...
        if (isFound)
        {
            var ret = this.Unregister(item.Key);
            if (ret)
            {
